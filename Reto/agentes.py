    ...
    def update_speed(self):
        ...
        for car in self.model.cars:
            if car != self:
                ...
                if abs(self.direction - car.direction) < 45 and (
                        ((self.x - car.x) * math.cos(angle)) + ((self.z - car.z) * math.sin(angle))) > 0:
        ...
        for semaphore in self.model.semaphores:
            if abs(self.direction - semaphore.direction) < 45 and (
                    (self.x * math.cos(angle)) + (self.z * math.sin(angle))) > 0:
...
parameters = {
    ...
    'steps': 10,  # Número de pasos de la simulación
}
